...
// ✅ Extract UserId function (Pehle se hi tha)
function extractUserId(req: any): string {
  if (req.user) {
    const id = req.user.userId || req.user.sub || req.user.id;
    if (id) return id;
...
export class UserController {
  ...
  constructor(
    ...
    // ✅ URL को बिना आगे के path के रखें (सिर्फ base URL)
  ...
  async getProfile(@Req() req: any, @Res() res: Response) {
    try {
      const userId = extractUserId(req);
      // ✅ यहाँ "/api/v1/users" हटा दिया है
      const response = await firstValueFrom(
        this.httpService.get(`${this.userServiceUrl}/me`, {  // <-- सिर्फ /me
          headers: { 
            'x-user-id': userId,
            'authorization': req.headers.authorization 
          },
  ...
  async updatePreferences(@Req() req: any, @Body() body: any, @Res() res: Response) {
    try {
      const userId = extractUserId(req);
      // ✅ यहाँ भी "/api/v1/users" हटा दिया है
      const response = await firstValueFrom(
        this.httpService.patch(`${this.userServiceUrl}/me/preferences`, body, {
          headers: { 
            'x-user-id': userId,
            'authorization': req.headers.authorization,
            'content-type': 'application/json'
