...
export class GamificationService {
  ...
  // Temporarily disabled - repository will be added back later
  constructor() {}

  async getXPSummary(userId: string) {
    console.log('⚠️ Gamification Service temporarily disabled - returning mock data');
    return {
      userId,
      totalXp: 0,
      currentLevel: 1,
  ...
  async getTransactions(userId: string, limit: number = 50) {
    console.log('⚠️ Gamification Service temporarily disabled - returning empty array');
    return [];
  }
  ...
  async handleLessonCompleted(event: any) {
    console.log('⚠️ Gamification Service temporarily disabled - event ignored');
    return { processed: false, reason: 'service temporarily disabled' };
  }
  ...
  private async addXP(userId: string, xp: number, eventType: string, eventId?: string, metadata?: any) {
    console.log('⚠️ Gamification Service temporarily disabled - addXP ignored');
    return;
  }

  private async updateStreak(userId: string) {
    console.log('⚠️ Gamification Service temporarily disabled - updateStreak ignored');
    return;
  }

  private async checkBadges(userId: string) {
    console.log('⚠️ Gamification Service temporarily disabled - checkBadges ignored');
    return;
  }

  private async updateAchievementProgress(userId: string, action: string, increment: number = 1) {
    console.log('⚠️ Gamification Service temporarily disabled - updateAchievementProgress ignored');
    return;
  }
