...
import {
  Mic, MicOff, ArrowLeft, Play, Square, Volume2,
  Loader2, CheckCircle2, AlertCircle, BarChart3,
  Clock, Brain, Zap, Target, Globe, Star
} from "lucide-react";
  ...
  useEffect(() => {
    const initialize = async () => {
      try {
        ...
        // (Since API might not be ready for /practice, we'll simulate here but keep it dynamic)
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white p-4 md:p-6 font-sans">
      <div className="max-w-5xl mx-auto space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400 flex items-center gap-2">
                <span className={`w-2 h-2 rounded-full ${healthStatus === "online" ? "bg-green-500 animate-pulse" : "bg-red-500"}`}></span>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <div className="bg-white/[0.03] border border-white/5 rounded-xl p-3 text-center">
              ...
              <p className={`text-sm font-semibold mt-1 capitalize ${
                sessionData.difficulty === "easy" ? "text-green-400" :
                sessionData.difficulty === "medium" ? "text-yellow-400" : "text-red-400"
              }`}>{sessionData.difficulty}</p>
            </div>
            <div className="bg-white/[0.03] border border-white/5 rounded-xl p-3 text-center">
            ...
            <div className="bg-white/[0.03] border border-white/5 rounded-xl p-3 text-center">
              ...
              <p className={`text-sm font-semibold mt-1 flex items-center justify-center gap-1 ${isListening ? "text-green-400" : "text-slate-400"}`}>
                {isListening ? <><Volume2 className="w-3 h-3 animate-pulse" /> Live</> : "Ready"}
              </p>
            </div>
          </div>
        )}
        ...
        <div className="bg-gradient-to-br from-slate-900/80 to-slate-800/80 border border-white/5 rounded-2xl p-6 md:p-8 backdrop-blur-sm">
          ...
          <div className="mb-6">
            <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Practice Text</h3>
          ...
          <div className="mb-6">
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Your Speech</h3>
              {isListening && (
          ...
          <div className="flex items-center justify-center gap-4">
            ...
            >
              ...
              <span>{isProcessing ? "Analyzing..." : isListening ? "Stop Recording" : "Start Practice"}</span>
            </button>
