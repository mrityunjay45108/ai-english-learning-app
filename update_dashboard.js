...
const dashPath = path.join(process.cwd(), "apps/web/src/app/dashboard/page.tsx");
const adminDashPath = path.join(process.cwd(), "apps/admin-ui/src/app/dashboard/page.tsx");

const fullDashboardCode = `"use client";
...
export default function DashboardPage() {
  ...
  const [chatMessages, setChatMessages] = useState<{ sender: "user" | "ai"; text: string }[]>([
    { sender: "ai", text: "Hello! I am your AI English Tutor. How can I help you practice today?" }
  ]);
  ...
  const sendAiChatMessage = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setChatMessages(prev => [...prev, { sender: "ai", text: data.reply || data.message || "Well done!" }]);
      } else {
        throw new Error("offline");
      }
    } catch (e) {
      setTimeout(() => {
        setChatMessages(prev => [...prev, { sender: "ai", text: "Excellent sentence! Try using complex clauses next time." }]);
      }, 500);
    } finally {
      setAiLoading(false);
    }
...
fs.writeFileSync(dashPath, fullDashboardCode);
if (fs.existsSync(adminDashPath)) fs.writeFileSync(adminDashPath, fullDashboardCode);
console.log("✅ Wrote realistic Ed-Tech checkout & invoice experience!");
